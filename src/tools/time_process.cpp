...
int timeStrToMinutes(const std::string& time_str) {
    ...
    if (time_str.size() != 5 && time_str.size() != 4) return -1;
    if (time_str[2] != ':' && time_str[1]!=':') return -1;
    for (size_t i = 0; i < time_str.size(); ++i) {
        if (time_str.size() == 5 && i == 2) continue;   // HH:MM 格式，跳过冒号
        if (time_str.size() == 4 && i == 1) continue;   // H:MM 格式，跳过冒号
        if (!std::isdigit(static_cast<unsigned char>(time_str[i]))) return -1;  // 非数字字符非法
    }
