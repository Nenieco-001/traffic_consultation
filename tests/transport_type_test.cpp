...
void test_city() {
    ...
    City city{1, "北京"};
    assert(city.id_ == 1);
    assert(city.name_ == "北京");

    City default_city{};
    assert(default_city.id_ == 0);
    assert(default_city.name_.empty());

    std::cout << "city.id_   = " << city.id_ << std::endl;
    std::cout << "city.name_ = " << city.name_ << std::endl;
...
void test_trip() {
    ...
    assert(train_trip.id_ == 1);
    assert(train_trip.type_ == TransportType::TRAIN);
    assert(train_trip.from_city_id_ == 1);
    assert(train_trip.to_city_id_ == 2);
    assert(train_trip.departure_time_ == 480);  // 08:00
    assert(train_trip.arrival_time_ == 780);    // 13:00
    assert(train_trip.price_ == 550);
    assert(train_trip.trip_number_ == "G1");

    // 飞机班次
    Trip flight_trip{13, TransportType::PLANE, 1, 2, 420, 510, 1200, "CA1234"};
    assert(flight_trip.id_ == 13);
    assert(flight_trip.type_ == TransportType::PLANE);
    assert(flight_trip.trip_number_ == "CA1234");

    std::cout << "train: id=" << train_trip.id_ << " " << train_trip.trip_number_ << " " << train_trip.departure_time_
              << "->" << train_trip.arrival_time_ << " ¥" << train_trip.price_ << std::endl;
    std::cout << "flight: id=" << flight_trip.id_ << " " << flight_trip.trip_number_ << " " << flight_trip.departure_time_
              << "->" << flight_trip.arrival_time_ << " ¥" << flight_trip.price_ << std::endl;
...
void test_segment() {
    ...
    assert(first_segment.trip_.id_ == 1);
    assert(first_segment.wait_time_ == 0);

    // 中转段等待时间 > 0
    Segment transfer_segment{trip, 30};
    assert(transfer_segment.wait_time_ == 30);

    std::cout << "first segment wait_time = " << first_segment.wait_time_ << std::endl;
    std::cout << "transfer segment wait_time = " << transfer_segment.wait_time_ << std::endl;
...
void test_path_single_segment() {
    ...
    assert(path.total_time_ == 300);  // 780 - 480
    assert(path.total_price_ == 550);
    assert(path.transfer_count_ == 0);
    assert(path.departure_time() == 480);
    assert(path.arrival_time() == 780);

    std::cout << "total_time=" << path.total_time_ << " total_price=" << path.total_price_
              << " transfer_count=" << path.transfer_count_ << std::endl;
...
void test_path_multi_segment() {
    ...
    assert(path.total_time_ == 450);
    assert(path.total_price_ == 840);
    ...
    // 验证每段详情
    assert(path.segments_[0].trip_.trip_number_ == "D633");
    assert(path.segments_[0].wait_time_ == 0);
    assert(path.segments_[1].trip_.trip_number_ == "G1001");
    assert(path.segments_[1].wait_time_ == 30);

    std::cout << "路线: 成都(5) -> 武汉(4) -> 广州(3)" << std::endl;
    std::cout << "  段1: " << path.segments_[0].trip_.trip_number_ << " " << path.segments_[0].trip_.departure_time_ << "->"
              << path.segments_[0].trip_.arrival_time_ << std::endl;
    std::cout << "  段2: " << path.segments_[1].trip_.trip_number_ << " " << path.segments_[1].trip_.departure_time_ << "->"
              << path.segments_[1].trip_.arrival_time_ << " (wait=" << path.segments_[1].wait_time_ << ")" << std::endl;
    std::cout << "  总耗时=" << path.total_time_ << "min" << " 总票价=¥" << path.total_price_
              << " 换乘=" << path.transfer_count_ << "次" << std::endl;
...
void test_path_cross_day() {
    ...
    assert(path.total_time_ == 1170);
    assert(path.total_price_ == 1400);
    ...
    for (size_t i = 0; i < path.segments_.size(); ++i) {
        const auto& s = path.segments_[i];
        std::cout << "  段" << (i + 1) << ": " << s.trip_.trip_number_ << " wait=" << s.wait_time_ << "min" << " "
                  << s.trip_.departure_time_ << "->" << s.trip_.arrival_time_ << std::endl;
    }
    std::cout << "  总耗时=" << path.total_time_ << "min" << " 总票价=¥" << path.total_price_
              << " 换乘=" << path.transfer_count_ << "次" << std::endl;
